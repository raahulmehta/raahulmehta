...
![Meta WhatsApp API](https://img.shields.io/badge/Meta_WhatsApp_Business_API-25D366?style=for-the-badge&logo=whatsapp&logoColor=white)
...
- ✅ **Desktop Apps** — Electron.js, packaged and shipped to production
...
- ☁️ **Edge & Serverless** — Cloudflare Workers, AWS Lambda, Vercel, Netlify
- 📊 **Monitoring** — Sentry, PM2, CloudWatch, uptime checks
...
- 🐳 **Containerized deployments** — Docker & Kubernetes running clean in any environment
...
|---|--------|---------------|-------|--------|
| 01 | [**Kitchen Solutions**](https://solutions.kitchen) | Architected and built end-to-end, solo, a multi-tenant SaaS catering platform for schools and corporate tenants, using a decoupled polyglot architecture with MongoDB for platform/authentication data and isolated PostgreSQL databases for tenant transactional data. Includes RBAC, MFA, SSO, custom domains with SSL management, real-time Socket.IO communication, event-driven notifications, payments, multilingual portals, dynamic tenant theming, Redis caching, Dockerized multi-instance deployment, NGINX performance tuning, GitHub Actions CI/CD, Sentry monitoring, AWS RDS/SES integrations, and Next.js multilingual marketing websites. | `React` `Next.js` `NestJS` `Fastify` `MongoDB` `PostgreSQL` `Prisma` `Mongoose` `Redis` `Socket.IO` `Docker` `NGINX` `GitHub Actions` `AWS RDS` `AWS SES` `Sentry` `Sanity` | 2026 |
| 02 | [**Duck Race Asia — Charity Event Platform**](https://duckraceasia.com) | Independently owned the complete product lifecycle from architecture and UI/UX planning to development, infrastructure, deployment, and production optimization. Built the Next.js public website, Fastify backend, and React admin CMS for a complete duck adoption and charity-event workflow, integrating Stripe payments with idempotent order processing, unique order IDs, receipt generation, OTP/email authentication, and Amazon SES transactional emails. Engineered real-time Socket.IO infrastructure for live adoption counts, sold-out indicators, banners, and event updates, alongside interactive SVG duck customization and social sharing with generated metadata. Dockerized and deployed four application instances on a Hostinger VPS with NGINX reverse proxy and performance tuning, optimizing the stack to handle traffic spikes of approximately 50,000+ concurrent users. | `Next.js` `Fastify` `React` `MongoDB` `Tailwind CSS` `Stripe` `Socket.IO` `Docker` `NGINX` `AWS SES` | Jul 2026 – Aug 2026 |
| 03 | **Daily Reports Analytics Engine** | Built end-to-end, solo, from architecture to deployment: a Slack-integrated analytics engine that ingests daily reports through the Slack API and webhooks, processes them on serverless Cloudflare Functions, and persists data across a hybrid storage layer (D1 + MongoDB + KV) for fast reads and flexible querying. Includes an admin dashboard on top. | `Fastify` `Slack API` `Cloudflare Functions` `Cloudflare D1` `MongoDB` `Cloudflare KV` | May 2026 |
| 04 | **Employee Tracking System** | Cross-platform desktop app built from scratch and shipped to production: Electron.js front end talking to a NestJS backend, deployed on a self-managed K3s Kubernetes cluster on AWS with a full GitHub Actions CI/CD pipeline handling build, test, and rollout. | `Electron.js` `NestJS` `K3s (Kubernetes)` `AWS` `GitHub Actions` | 2026 |
| 05 | [**Legacy Life Counseling**](https://legacylifecounseling.vercel.app/) | Premium portfolio-style site for a Canada-based client, built and deployed solo. Sanity CMS wired in so the client can update content themselves, transactional email via Resend, clean SEO-friendly structure and subtle motion throughout. | `Next.js` `Sanity CMS` `Resend` | May 2026 |
| 06 | [**LGXNDS**](https://lgxnds.com/) | Edge API proxy built on Cloudflare Workers, integrated with Seal Subscriptions to route and manage subscription-related requests at the edge — designed, built, and deployed from scratch. | `Hono.js` `Cloudflare Workers` | March 2026 |
| 07 | [**Bornvia**](https://www.bornvia.com) | Scroll-driven storytelling site: hand-built GSAP ScrollTrigger animation sequences wired into Framer via custom code components, taking the design from static mockup to a fully interactive, pinned-scroll experience. | `Framer` `GSAP` `ScrollTrigger` | Feb 2026 |
| 08 | **TSTN** | Rewards and loyalty platform built from the ground up — landing page, dashboard UI, and GSAP-driven micro-interactions layered on top of the core reward-tracking logic. | `GSAP` | Feb 2026 |
| 09 | [**Flowpouch**](https://flowpouch.com) | Cloudflare Worker built and deployed to bridge ProsperStack into FlowPouch's subscription flow, improving cancellation and retention handling without touching the core app. | `Cloudflare Workers` `Hono.js` `Wrangler` | Jan–Feb 2026 |
| 10 | **WhatsApp Chat Bot** | Webhook-based WhatsApp Business bot built end-to-end on the Meta Cloud API, edge-deployed on Cloudflare Workers, driving an AI pipeline that handles personal-assistant tasks and business queries in real time. | `Meta Cloud API` `Cloudflare Workers` `Hono.js` `Wrangler` | Feb 2026 |
| 11 | [**iCreate Media**](https://icmnext.vercel.app) | Agency website designed and built solo, from static design to deployed site: GSAP scroll-triggered animation, Three.js WebGL scenes for the hero, fully responsive and SEO-optimised. | `Next.js` `GSAP` `Three.js` `Tailwind` | Dec 2025–Jan 2026 |
| 12 | [**LYM Digital**](https://lymdigital.com) | High-performance agency site shipped from scratch to production: GSAP scroll animation, Vercel serverless functions powering the contact form, and Next.js SEO tuning throughout. | `Next.js` `GSAP` `Framer Motion` | Dec 2025–Jan 2026 |
| 13 | [**Fertility Minds**](https://apps.apple.com/in/app/fertility-minds/id6786222520) | Cross-platform mobile app taken from zero to live on both the App Store and Google Play: auth, Stripe subscriptions, real-time Socket.io chat, push notifications, social posting, a custom CMS/CRM with a React admin panel, plus store-listing and release management. | `React Native` `Node.js` `MongoDB` `Socket.io` `Stripe` | Oct 2025–Feb 2026 |
| 14 | [**Al Khaleej Takaful (AKTI)**](https://www.alkhaleej.com) | Bilingual EN/AR insurance platform built solo with full RTL/LTR support, a complex state-persistent multi-step form validated with React Hook Form, and smooth motion throughout — designed, built, and deployed end-to-end. | `Vite` `React` `Tailwind` `GSAP` `i18n` | Aug–Dec 2025 |
| 15 | **Trade Assurance** | Global trade platform built from scratch: secure auth, order management, multi-step product/service listing flows, real-time notifications, and admin dashboards — architected, built, and deployed solo. | `Next.js` `Express` `MongoDB` `Tailwind` | Jul 2025–Feb 2026 |
| 16 | [**Vetrinamia**](https://vetrinamia.com) | Real-time analytics engine shipped end-to-end: WebSocket streams feeding a live admin dashboard, Shopify marketplace integration, deployed on a load-balanced VPS behind NGINX. | `MERN` `WebSockets` `NGINX` `Shopify` | Jul–Aug 2025 |
| 17 | [**Trans WeGo**](https://www.transwego.ch) | Switzerland–Germany logistics platform built solo, from data model to live deployment: shipment bidding, a cost-sharing algorithm, Google Maps geo-aggregation, and real-time push plus group chat. | `Next.js` `Node.js` `MongoDB` `Socket.io` `Google Maps` | Nov 2024–Jun 2025 |
| 18 | [**Connect You**](https://connectyou.global) | Global coaching platform taken from scratch to production on a VPS: Stripe payments, real-time chat, Google Maps + Calendar integration, HubSpot sync, timezone-aware slot booking, and payout/refund workflows. | `MERN` `Next.js` `Socket.io` `Stripe` `HubSpot` | Aug 2024–Sep 2025 |
| 19 | **Yacht Buddy** | Frontend build focused on advanced scroll animation and interaction design, from static comps to a fully animated live site. | `React` `GSAP` | Sep 2024 |
| 20 | **Supertiny Marketing Agency** | Dev agency portfolio built and deployed solo on a VPS, with GSAP animation and SEO tuning baked in from the start. | `React` `Vite` `Tailwind` `GSAP` | Sep 2024 |
| 21 | [**NaveesInfoTech**](https://naveesinfotech.com/) | Marketing agency portfolio designed, built, and deployed to a VPS, with GSAP animation tuned for lead generation. | `React` `Vite` `Tailwind` `GSAP` | Aug 2024 |
<!-- PROJECTS-TABLE:END -->
...
Docker, Kubernetes, GitHub Actions CI/CD, Nginx, VPS, Edge — apps that go live cleanly and stay live
